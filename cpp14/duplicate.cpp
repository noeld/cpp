...
int main (int argc, char const *argv[])
{
	...
	try {
		...
		std::thread obs([&processed, &captureoutput, &digested, files_added]() {
			while(1) {
		...
		for(const auto& s : sizemap) {
			...
			for(const auto& e : hm) {
				if (e.second.size() <= 1)
					continue;
				save_potential += s.first * (e.second.size()-1);
				captureoutput = 1;
...
/*		ThreadPool::ThreadPool tp(1);
		...
		for (int i = 0; i<1; ++i) {
			ThreadPool::ThreadPool::taskptr_t t(new ThreadPool::LambdaTask<std::function<void(void)>>([i]{ std::cout << "LambdaTask " << i << std::endl;}));
			tp.tasks().enqueue(std::move(t));
		}
		tp.stopWorkers();
		std::cout << "Ergebnis" << std::endl;
		ThreadPool::ThreadPool::taskptr_t t;
		while(tp.finished().try_dequeue(t)) {
			std::cout << __FUNCTION__ << std::endl;
			std::cout << t->duration_net().count() << std::endl;
		}*/
